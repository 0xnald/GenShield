...
def main():
    ...
    if not os.path.exists(address_path):
        print(f"Error: {address_path} not found. Please run deploy_bradbury.py first.")
        return
        
    with open(address_path, "r") as f:
        addresses = json.load(f)
        
    contract_address = addresses.get("bradbury")
    if not contract_address:
        print("Error: Bradbury contract address not found in deployed_addresses.json.")
        return
        
    print(f"Loaded GenShield Bradbury contract address: {contract_address}")

    print("Initializing GenLayer client for Bradbury Testnet...")
    account = genlayer_py.create_account(private_key)
    client = genlayer_py.create_client(chain=genlayer_py.testnet_bradbury, account=account)
    ...
    print("\n--- Running Live Security Audit on Bradbury ---")
    sample_code = (
